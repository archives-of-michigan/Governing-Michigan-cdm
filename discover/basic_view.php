<div  id="item-view">
  <div class="wrapper mod">
    ...
    <div class="item-preview">
      ...
      <? elseif($is_image): ?>
        <a href="/custom/discover_item_viewer.php?CISOROOT=<?= $alias ?>&amp;CISOPTR=<?= $itnum ?>&amp;CISOSHOW=<?= $itnum ?>&amp;search=<?= $seek_search_params ?>">
          <img src="<?= $file_url ?>" />
        </a>
      <? elseif(isset($pageptr) && $pageptr != 0): ?>
        <b><a href="<?= $file_url ?>&amp;CISOPAGE=<?= $pageptr + isSearchHit($pageptr) ?>">View Document</a>
      <? elseif($file_extension == 'url'): ?>
        <div style="text-align: center; margin: 10px; font-size: 14px;"><a href="<?= $file_url ?>">Visit URL</a></div>
      <? else: ?>
